...
По умолчанию сканер слушает **топ-100 пар** на спот-рынке и
**топ-100 пар** на perp-рынке — всё, что есть минимум на 5 из 7
наших бирж и отсортировано по агрегированному 24-часовому
USD-объёму. Полный список и обоснование — в
[`docs/universe-100.md`](docs/universe-100.md), машиночитаемый —
в `docs/universe-100.json`. Файл `settings.example.yaml` уже
содержит этот универсум.
...
symbols:
  spot:
    ...
    - XRPUSDT
    - SOLUSDT
    # … ещё 96 пар, см. docs/universe-100.md
  perp:
    - BTCUSDT
    - ETHUSDT
    - SOLUSDT
...
Формат канонический: `БАЗА+КВОУТ` без разделителя (`BTCUSDT`, не
`BTC/USDT` и не `BTC-USDT`). Листенер каждой биржи сам переводит в
нативный формат (`BTC_USDT` для Gate.io, `BTC-USDT` для KuCoin,
`XBTUSDTM` для KuCoin perp).

**Как был выбран этот универсум.** Скрипт
[`scripts/discover_universe.py`](scripts/discover_universe.py)
ходит в REST-эндпоинты тикеров всех 7 бирж × 2 рынка, нормализует
названия (XBT → BTC, `_` → пусто, `-` → пусто), агрегирует объём
по парам и берёт топ-N из тех, что доступны минимум на 5
биржах — гарантия, что у любого арб-сигнала будут как минимум 4
независимых контрагента.

```bash
# пересобрать универсум (например, на топ-50)
python scripts/discover_universe.py --top 50 --min-venues 5 \
  --out-json docs/universe-100.json
```

После пересборки скопируй YAML-секцию из stdout в свой
`settings.yaml` (или прямо в `settings.example.yaml`).

**Лимиты подписки и шардинг.** Не все биржи переваривают 100
стримов на одно WebSocket-соединение. Эти три листенера
автоматически дробят подписку прозрачно для пользователя:

* **Bybit spot** — `op:subscribe` принимает максимум 10 args за
  одно сообщение, поэтому листенер шлёт 10 sub-фреймов на коннекте.
* **MEXC spot/perp** — лимит 30 стримов/коннект, поэтому
  `run_mexc()` поднимает 4 параллельных WS по 25 стримов и
  объединяет тики в одну книгу `prices`.

**Уменьшить универсум.** Если хочешь меньшую нагрузку (например,
для разработки или слабого VPS), создай собственный
`settings.yaml` с урезанным списком — он не gitignored для
зеркального переопределения, и сканер автоматически возьмёт его
вместо `settings.example.yaml`. Минимум — 1 пара, ограничения нет.

**Кардинальность метрик.** При 100 парах метрика
`arb_signals_total{market,symbol,buy_ex,sell_ex}` имеет
~9 800 серий (100 × 7 × 7 × 2) — для Prometheus это копейки,
дашборд не тормозит.
